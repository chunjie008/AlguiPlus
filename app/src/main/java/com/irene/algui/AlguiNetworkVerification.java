...
public class AlguiNetworkVerification {
    ...
    //网络验证配置 (微验)
    private final boolean IO=true;//网络验证总开关(调试用)
    ...
    private final int successStatusCode=200;//成功状态码
    ...
    //获取机器码
    private String getMachineCodeBack() {
    ...
    public String getMachineCode() {
        ...
        try {
            // 执行su命令获取ANDROID_ID
            Process process = Runtime.getRuntime().exec(new String[]{"su", "-c", "settings get secure android_id"});
            BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()));
            androidId = reader.readLine().trim(); // 读取输出
            process.waitFor();
    ...
    private void kamiInspect() {
        ...
        //文本布局增加一个文本
        AlGui.GUI(context).addTextView(textLayout2, Html.fromHtml("没有卡密？联系qq:3353484607"), 11, 0xFFE0E0E0, null);
